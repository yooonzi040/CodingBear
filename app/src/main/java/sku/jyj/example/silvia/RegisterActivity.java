...
public class RegisterActivity extends AppCompatActivity {
    ...
    private EditText input_id, input_pw, input_name, input_phoneno;
    ...
    public void sendServer(final String dateMessage){ // 서버로 데이터 전송하기 위한 함수
        class sendData extends AsyncTask<Void, Void, String> { // 백그라운드 쓰레드 생성
            ...
            protected String doInBackground(Void... voids) {
                try {
                    ...
                    jsonInput.put("userName", input_name.getText().toString()); // [vscode-app.py] JSON 객체에 데이터 추가 (name)
                    jsonInput.put("userPhoneno", input_phoneno.getText().toString()); // [vscode-app.py] JSON 객체에 데이터 추가 (phoneno)
                    jsonInput.put("userBirth", dateMessage);
